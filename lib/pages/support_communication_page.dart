...
class _SupportCommunicationPageState extends State<SupportCommunicationPage> {
  ...
  Future<void> _loadRequestReason() async {
    final request = await PocketBaseService.instance.pb
  ...
  Future<void> _loadAttachments() async {
    ...
    for (final record in result.items) {
      final messageId = _relationId(record.data['support_request_message_id']);
      if (messageId == null || !messageIds.contains(messageId)) continue;

      final fileName = _firstFileName(record.data['photo']) ?? 'Файл';

      final url = _fileUrl(
        collectionName: 'support_request_attachments',
        recordId: record.id,
    ...
    try {
      ...
      if (hasFile) {
        ...
        await PocketBaseService.instance.pb
            .collection('support_request_attachments')
            .create(
              body: {'support_request_message_id': message.id},
              files: [
